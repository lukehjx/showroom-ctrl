...
async def sync_resources() -> dict:
    ...
    try:
        ...
        # 用 (scene_id, terminal_id, exhibit_id) 作为唯一键
        # 同一资源在多个专场×终端出现时全部保留
            ...
            for t in terminals:
                ...
                for res in t.get("exhibitionResourceList", []):
                    ...
                    if key not in seen_keys:
                        seen_keys.add(key)
                        enriched = dict(res)
                        enriched["_terminal_id"] = tid
                        enriched["_terminal_name"] = t.get("hostName", "")
                        enriched["_terminal_ip"] = t.get("hostIp", "")
                        enriched["_scene_id"] = sid
                        all_resources.append(enriched)

        logger.info(f"Collected {len(all_resources)} resource-scene-terminal records from {len(scenes_list)} scenes")
        ...
        async with async_session() as session:
            ...
            sync_log.status = "success"
            sync_log.records_count = len(all_resources)
            session.add(sync_log)
            await session.commit()

        return {"type": "resources", "count": len(all_resources)}
...
async def sync_commands() -> dict:
    ...
    try:
        ...
        async with async_session() as session:
            await session.execute(delete(CloudCommand).where(CloudCommand.group_name != "组策略"))
...
async def sync_group_policies() -> dict:
    """
    同步组策略命令（0_all_xxx 格式 TCP 指令）
    接口：GET /pc/resource/api/v1/group/?page=1&rows=100&releaseState=
    需要 SESSION cookie（PC 端管理员权限），从 system_config.cloud.session_cookie 读取
    """
    session_cookie = await get_config("cloud.session_cookie")
    if not session_cookie:
        logger.info("cloud.session_cookie not configured, skipping group policy sync")
        return {"type": "group_policies", "count": 0, "skipped": True}

    api_url = await get_config("cloud.api_url") or "http://112.20.77.18:7772"
    # 内网 /pc/ 路径走 nginx 代理（80端口）
    pc_base = "http://192.168.18.65"
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            r = await client.get(
                f"{pc_base}/pc/resource/api/v1/group/",
                params={"page": 1, "rows": 100, "releaseState": ""},
                headers=headers,
            )
            d = r.json()

        if d.get("code") != 200:
            return {"type": "group_policies", "count": 0, "error": d.get("msg", "403")}

        items = d["data"]["list"]
        valid = [
            i for i in items
            if i.get("releaseState") == "1"
            and i.get("wholeOrderMsg")
            and "_" in str(i.get("wholeOrderMsg", ""))
            and len(str(i.get("wholeOrderMsg", ""))) >= 5
        ]
        logger.info(f"Fetched {len(valid)} valid group policies")

        async with async_session() as db:
            # 只删除 group_name=组策略 的旧数据
            from sqlalchemy import delete as sa_delete
            await db.execute(
                sa_delete(CloudCommand).where(CloudCommand.group_name == "组策略")
            )
            for item in valid:
                db.add(CloudCommand(
                    command_id=item["id"],
                    name=item["wholeOrderName"],
                    command_str=item["wholeOrderMsg"],
                    protocol_type="tcp",
                    is_hex=False,
                    group_name="组策略",
                    encoding="utf-8",
                    area_name=item.get("exhibitionAreaName") or "全馆",
                    raw_data=item,
                    synced_at=datetime.now(),
                ))
            await db.commit()

        return {"type": "group_policies", "count": len(valid)}

    except Exception as e:
        logger.error(f"sync_group_policies failed: {e}")
        return {"type": "group_policies", "count": 0, "error": str(e)}
